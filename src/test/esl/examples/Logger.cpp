...
void loggerInitialize() {
	...
	/* setting default */
	esl::logging::setLevel(esl::logging::Level::WARN, "*");

	//esl::logging::setLevel(esl::logging::Level::INFO, "AAA::BBB::*");
	//esl::logging::setLevel(esl::logging::Level::DEBUG, "AAA::BBB::CCC::*");
}

void loggerReplay() {
	appenderMemBuffer.getLayout().setShowTypeName(false);
	std::vector<std::tuple<esl::logging::Location, std::string>> buffer = appenderMemBuffer.getBuffer();
